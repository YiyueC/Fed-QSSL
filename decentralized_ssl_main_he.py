...
if __name__ == "__main__":
    ...
    cbit_local = [4, 4, 6, 6, 6, 8, 8, 8, 12, 12]
    weights_list = [i/sum(cbit_local) for i in cbit_local ]
    print("weights check:", weights_list)
    ...
    global_model_deq = get_global_model(args, 32, train_dataset).to(device)

    if args.distributed_training:
        global_model_deq = DDP(global_model_deq)
    else:
        global_model_deq = torch.nn.DataParallel(global_model_deq)
    global_model_deq.train()
    ...
    global_dequan_update = GlobalUpdate(
            args=args,
            dataset=public_train_dataset,
            logger=logger,
            output_dir=model_output_dir,
    )
    global_dequan_update.init_model(global_model_deq)
    ...
    for epoch in tqdm(range(start_epoch, total_epochs)):
        ...
        de_quan_loss = []
        for idx in range(args.num_users):
            global_model_dq = global_dequan_update
            w, loss = global_model_dq.update_ssl_weights(
                model = copy.deepcopy(local_models[idx]),
                global_round = epoch,
                load_weights = local_weights[idx],
                lr = lr,
                cbit = 32
            )
            de_quan_loss.append(loss)
            local_weights[idx] = global_model_dq.model.state_dict()
        print("De-quantization done!")
        print("De-quan loss:", de_quan_loss)

        deq_list = [np.exp(-de_quan_loss[i]) for i in range(len(de_quan_loss))]
        # Add drop low perf models
        # min_idx = deq_list.index(min(deq_list))
        # deq_list[min_idx] = 0
        deq_list = [i/sum(deq_list) for i in deq_list]
        print("De-quan weight:", deq_list)

        # update global weights
        # global_weights = average_weights(local_weights)
        global_weights = weighted_average_weights(local_weights, deq_list)
        ...
        for idx in idxs_users:
            ...
            w, loss = global_model_q.update_ssl_weights(
                model=copy.deepcopy(local_models[idx]),
                global_round=epoch,
                load_weights = global_weights,
                lr=lr,
                cbit = cbit_local[idx]
            )
            local_weights_req.append(global_model_q.model.state_dict() )
        ...
        if args.average_without_bn:
            for i in range(args.num_users):
                local_models[i] = load_weights_without_batchnorm(
                    local_models[i], local_weights_req[i]
                )
        else:
            for i in range(args.num_users):
                local_models[i] = load_weights(local_models[i], local_weights_req[i])
        global_model.load_state_dict(global_weights)
    ...
    for idx in range(args.num_users):
        ...
        test_acc_l = global_repr_local_classifier(args, cbit_local[idx], num_class, local_models_n[idx], train_dataset_idx, test_dataset_idx, args.finetuning_epoch)
        print("client", idx, cbit_local[idx], test_acc_l)

    # evaluate representations
    print("evaluating representations: ", model_output_dir)
    test_acc = global_repr_global_classifier(args, cbit, num_class, global_model, args.finetuning_epoch)
