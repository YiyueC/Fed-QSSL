...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, wbit=1, abit=1, num_classes=10):
        ...
        self.head = nn.Sequential(
                ...
                conv3x3(3, 64, wbit=wbit, abit=abit, stride=1),
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True),
        )
...
class CNN_v1(nn.Module):
    def __init__(self, wbit = 1, abit = 1, num_classes=10):
        super(CNN_v1, self).__init__()
        self.in_planes = 64

        self.layer1 = nn.Sequential(
          Conv(3, 128, kernel_size = 3, stride = 1, padding = 1, bias = False, nbit_w = wbit, nbit_a = abit),
          nn.BatchNorm2d(128),
          nn.ReLU(inplace = True),
          Conv(128, 128, kernel_size = 3, stride = 1, padding = 1, bias = False, nbit_w = wbit, nbit_a = abit),
        ...
        self.layer2 = nn.Sequential(
          Conv(128, 256, kernel_size = 3, stride = 1, padding = 1, bias = False, nbit_w = wbit, nbit_a = abit),
          nn.BatchNorm2d(256),
          nn.ReLU(inplace = True),
          Conv(256, 256, kernel_size = 3, stride = 1, padding = 1, bias = False, nbit_w = wbit, nbit_a = abit),
          nn.MaxPool2d(kernel_size = 3, stride = 2, padding = 1),
          nn.BatchNorm2d(256),
          nn.ReLU(inplace = True)
        )
        ...
        self.layer3 = nn.Sequential(
          Conv(256, 512, kernel_size = 3, stride = 1, padding = 1, bias = False, nbit_w = wbit, nbit_a = abit),
          nn.BatchNorm2d(512),
          nn.ReLU(inplace = True),
          Conv(512, 512, kernel_size = 3, stride = 1, padding = 1, bias = False, nbit_w = wbit, nbit_a = abit),
          nn.MaxPool2d(kernel_size = 3, stride = 2, padding = 1),
          nn.BatchNorm2d(512),
          nn.ReLU(inplace = True)
        )
        ...
        self.layer4 = nn.Sequential(
          nn.Flatten(),
          Linear(8192, 1024, nbit_w = wbit, nbit_a = abit),
          nn.BatchNorm1d(1024),
          nn.ReLU(inplace = True),
          Linear(1024, 512, nbit_w = wbit, nbit_a = abit),
          nn.BatchNorm1d(512),
          # nn.ReLU(inplace = True)
        )

        # self.tail = nn.Sequential(
        #         Linear(512, num_classes, nbit_w = wbit, nbit_a = abit),
    ...
    def init_w(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                # print("weight check before:", m.weight.data)
                m.weight.data = torch.sign(m.weight.data)*torch.mean(m.weight.data)
                # print("weight check after:", m.weight.data)
                if m.bias is not None:
                    nn.init.zeros_(m.bias)
            elif isinstance(m, nn.BatchNorm2d):
                nn.init.ones_(m.weight)
    ...
    def forward(self, x):
        ...
        out = self.layer1(x)
        # print("l1 out size:", out.size())
        out = self.layer2(out)
        # print("l2 out size:", out.size())
        out = self.layer3(out)
        # print("l3 out size:", out.size())
        out = self.layer4(out)
        # out = self.tail(out)
        return out

def CNN(wbit, abit):
    return CNN_v1(wbit = wbit, abit = abit )
