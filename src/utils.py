...
def global_repr_global_classifier(args, cbit, num_class, global_model, test_epoch=60):
    ...
    if args.ssl_method == "mae":
        ...
        global_model_classifer = ResNetCifarClassifier(args=args, cbit = cbit, num_class=num_class)
...
def global_repr_local_classifier(args, cbit, num_class, global_model, trainloader, testloader, test_epoch=60):
    # global representation, global classifier
    from models import ResNetCifarClassifier
    from update import test_inference_loader
    ...
    for epoch_idx in tqdm(range(test_epoch)):
        ...
        for batch_idx, data in enumerate(trainloader):
            ...
            weights_before = global_model_classifer.state_dict()
            weights_quan = dorefa_quan(weights_before, cbit)
            global_model_classifer.state_dict(weights_quan)
        ...
        loss_avg = sum(batch_loss) / len(batch_loss)
        test_acc, test_loss = test_inference_loader(args, global_model_classifer, testloader)
        if test_acc > best_acc:
            best_acc = test_acc
        print("\n Downstream Train loss: {} Acc: {}".format(loss_avg, best_acc))
    return best_acc
...
def get_global_model(args, cbit, train_dataset):
    from models import SimCLR
    from simsiam import SimSiam

    if args.ssl_method == "simclr":
        global_model = SimCLR(args=args, cbit=cbit)
...
def get_local_model(args, cbit):
    from models import SimCLR
    from simsiam import SimSiam

    if args.ssl_method == "simclr":
        local_model = SimCLR(args=args, cbit=cbit)
    elif args.ssl_method == "simsiam":
        local_model = SimSiam(args=args)
    elif args.ssl_method == "mae":
        local_model = MAE_ViT(mask_ratio=0.75)
    return local_model
...
def get_dataset(args, **kwargs):
    ...
    if "cifar" in args.dataset:
        ...
        train_dataset.data = train_dataset.data[:45000]
        train_dataset.targets = train_dataset.targets[:45000]
        ...
        public_train_dataset = train_dataset_func(
            root=data_dir,
            train=True,
            download=True,
            transform=train_transform_,
        )
        public_train_dataset.data = public_train_dataset.data[45000:]
        public_train_dataset.targets = public_train_dataset.targets[45000:]
    ...
    if args.iid:
        ...
        if args.dirichlet:
            ...
            print("train dataset length:", train_dataset.data.shape)
    ...
    return train_dataset, test_dataset, public_train_dataset, user_groups, memory_dataset, test_user_groups
...
def weighted_average_weights(w, weights):
    ...
    for key in w_avg.keys():
        for i in range(len(w)):
            if i == 0:
                w_avg[key] = torch.mul(w_avg[key], weights[0])
            else:
...
def accuracy(output, target, topk=(1,)):
    ...
    ret = []
    for k in topk:
        correct_k = correct[:k].view(k, -1).float().sum(dim=0, keepdim=True)
        correct_k = sum(correct_k)
        ret.append(correct_k.mul_(1. / batch_size))
    return ret

def save_checkpoint(state, is_best = True, fpath = 'checkpoint.pth.tar'):
